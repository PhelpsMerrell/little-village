...
const ROOM_RESPAWN_INTERVAL := 30.0  ## seconds between resource respawns in stone/fish rooms
const ROOM_RESPAWN_MAX_STONE := 8    ## max stones per stone room
const ROOM_RESPAWN_MAX_FISH := 4     ## max fish per river room

var _river_room_ids: Array = []  ## populated after map generation
var _portal_pairs: Dictionary = {}  ## room_id -> partner_room_id
var _portals: Array = []  ## portal node references

var _river_fish_timer: float = 0.0
var _room_respawn_timer: float = 0.0
...
var churches: Array = []
var universities: Array = []
var farms: Array = []
var town_halls: Array = []

var _player: Node = null  ## PlayerController instance

## Spatial room lookup grid: cell_key (int) -> room_id
var _room_grid: Dictionary = {}
var _room_grid_cell_size: float = 64.0  ## Pixels per grid cell

## Staggered brain update
var _brain_batch_index: int = 0
const BRAIN_BATCH_SIZE := 32  ## Max villagers to update brain context per frame
...
var _university_scene: PackedScene = preload("res://scenes/university.tscn")
var _farm_scene: PackedScene = preload("res://scenes/farm.tscn")
var _town_hall_scene: PackedScene = preload("res://scenes/town_hall.tscn")
...
var _name_generation_tracker: Dictionary = {}  ## base_name -> next generation number
var _default_name_index: int = 0  ## Tracks which default name to assign next
var _tutorial_enemies_killed: int = 0  ## Track for tutorial phase skip-ahead
var _ai_controllers: Array = []  ## AIController instances for AI factions

const DEFAULT_VILLAGER_NAMES: Array = [
	"Goeffred", "Stanuel", "Himothy", "Abroham", "Pichael", "Mortimus", "Freaux",
]


func _assign_default_name(v: Node) -> void:
	## Assign a unique default name from the roster to a starting villager.
	if _default_name_index < DEFAULT_VILLAGER_NAMES.size():
		v.villager_name = DEFAULT_VILLAGER_NAMES[_default_name_index]
	else:
		# Overflow: cycle with a suffix
		var base_idx: int = _default_name_index % DEFAULT_VILLAGER_NAMES.size()
		var cycle: int = _default_name_index / DEFAULT_VILLAGER_NAMES.size() + 1
		v.villager_name = "%s_%d" % [DEFAULT_VILLAGER_NAMES[base_idx], cycle]
	v.generation = 1
	_default_name_index += 1


func _gen_descendant_name(parent: Node) -> Dictionary:
	## Returns {name, generation} for a new villager descended from parent.
	var base_name: String = parent.villager_name
	if base_name.is_empty():
		base_name = parent.color_type.capitalize()
	var next_gen: int = parent.generation + 1
	return {"name": base_name, "generation": next_gen}


func _gen_villager_name(color_id: String) -> String:
	## Fallback name for villagers without lineage context.
...
func _read_lobby_config() -> void:
	...
	if FactionManager.has_meta("ai_factions"):
		FactionManager.remove_meta("ai_factions")
...
func _generate_map() -> void:
	...
	var containers := {
		...
		"portals": _portals_container,
		"town_halls": _town_halls_container,
	}
	var scenes := {
		...
		"portal": preload("res://scenes/portal.tscn"),
		"town_hall": _town_hall_scene,
	}
	if TutorialManager.active:
	...
	_portal_pairs = _map_gen.portal_pairs.duplicate()
...
func _collect_all() -> void:
	...
	_portals.clear()
	for p in _portals_container.get_children():
		_portals.append(p)

	universities.clear()
	for u in _universities_container.get_children():
		universities.append(u)

	farms.clear()
	for f in _farms_container.get_children():
		farms.append(f)

	town_halls.clear()
	for th in _town_halls_container.get_children():
		town_halls.append(th)
...
func _collect_villagers() -> void:
	...
	for v in _villager_container.get_children():
		...
		if v.villager_name == "" and str(v.color_type) != "magic_orb":
			_assign_default_name(v)
...
func _process(delta: float) -> void:
	...
	_process_room_respawns(delta)
	_process_portals()
	_process_farm_grain(delta)

	# AI opponents
	for ai in _ai_controllers:
		ai.ai_process(delta)

	# Walls only exist in tutorial/sandbox
	var wall_data: Array = []
	for w in walls:
		wall_data.append({"room_a": w.room_a_id, "room_b": w.room_b_id, "is_open": w.is_open})

	# Door breaking only relevant in tutorial/sandbox (walls present)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# Pending attack/stun: click on an enemy-faction villager OR enemy entity
	if _hud.get_pending_command() in ["attack", "stun"] and _player.has_selection():
		...
		for v in villagers:
			...
			if d < float(v.radius) + 10.0 and d < best_d:
				best_d = d
				target_node = v
		# Check enemies (black circle enemies, demons, zombies)
		for e in enemies + night_enemies:
			if not is_instance_valid(e) or e.is_dead or not e.visible:
				continue
			var d: float = click_pos.distance_to(e.global_position)
			if d < float(e.radius) + 10.0 and d < best_d:
				best_d = d
				target_node = e
		# Check breakable obstacles (attack = move red to break them)
		if target_node == null and combat_cmd == "attack":
			for room in rooms:
				for child in room.get_children():
					if child.has_method("check_break") and child.get("is_intact") == true:
						var obs_center: Vector2 = child.global_position + child.wall_size * 0.5
						var d: float = click_pos.distance_to(obs_center)
						if d < 60.0:
							# Move reds to obstacle — auto-break on proximity
							_player.command_move_to(obs_center)
							_hud.clear_pending_command()
							get_viewport().set_input_as_handled()
							return
		if target_node:
			if combat_cmd == "attack":
				_player.command_attack_target(target_node)
			else:
				_player.command_stun_target(target_node)
		else:
			EventFeed.push("No target at that location.", Color(0.7, 0.5, 0.3))
		_hud.clear_pending_command()
		get_viewport().set_input_as_handled()
		return

	# Resource click-and-assign (stone → yellow, fish → blue)
	if _player.has_resource_selection():
	...
	for c in collectables:
		...
		if click_pos.distance_to(c.global_position) < 20.0:
			_player.set_resource_selection(c, c.resource_type)
			get_viewport().set_input_as_handled()
			return
	for f in fish_spots:
		if not is_instance_valid(f) or f.collected:
	...
	# Building click / house targeting
	...
	all_buildings.append_array(town_halls)

	var clicked_building: Node = _get_clicked_building(click_pos, all_buildings)
	if clicked_building:
		if _hud.get_pending_command() == "house":
			var rid: int = _room_id_at(clicked_building.global_position)
			var room_owner: int = RoomOwnership.ownership.get(rid, -1)
			if _player.command_house_in_target_building(clicked_building, room_owner):
				_hud.clear_pending_command()
				get_viewport().set_input_as_handled()
				return
			EventFeed.push("Pick your own home or church in your owned room.", Color(0.9, 0.6, 0.3))
			get_viewport().set_input_as_handled()
			return
...
func _place_building(item_id: String, pos: Vector2) -> void:
	...
	elif item_id == "university":
		var u = _university_scene.instantiate()
		_universities_container.add_child(u)
		u.global_position = pos
		u.placed_by_faction = fid
		universities.append(u)
		EventFeed.push("University built.", Color(0.9, 0.7, 0.3))
	elif item_id == "farm":
		var f = _farm_scene.instantiate()
		_farms_container.add_child(f)
		f.global_position = pos
		f.placed_by_faction = fid
		farms.append(f)
		EventFeed.push("Farm built.", Color(0.7, 0.8, 0.3))
	...
	match cmd_type:
		...
		"sell":
			...
			if building in homes:
				item_id = "house"
			elif building in churches:
				item_id = "church"
			elif building in banks:
				item_id = "bank"
			elif building in fishing_huts:
				item_id = "fishing_hut"
			elif building in universities:
				item_id = "university"
			elif building in farms:
				item_id = "farm"
			...
			if building in banks:
				banks.erase(building)
			if building in fishing_huts:
				fishing_huts.erase(building)
			if building in universities:
				universities.erase(building)
			if building in farms:
...
func _eliminate_faction(eliminated_fid: int, captor_fid: int) -> void:
	...
	for b in homes + churches + banks + fishing_huts + town_halls:
...
func _find_entity_by_net_id(nid: int) -> Node:
	## Search villagers, enemies, and night enemies by net_id.
	for v in villagers:
		if is_instance_valid(v) and v.net_id == nid:
			return v
	for e in enemies:
		if is_instance_valid(e) and e.net_id == nid:
			return e
	for ne in night_enemies:
		if is_instance_valid(ne) and ne.net_id == nid:
			return ne
	return null
...
func _apply_net_command(cmd: Dictionary) -> void:
	...
	match cmd_type:
		...
		"house_target":
			var bpos := Vector2(float(cmd.get("bx", 0.0)), float(cmd.get("by", 0.0)))
			var target_building: Node = null
			for b in homes + churches + town_halls:
				if is_instance_valid(b) and b.global_position.distance_to(bpos) < 60.0:
					target_building = b
					break
			if target_building:
				for nid in net_ids:
					var v: Node = _find_villager_by_net_id(int(nid))
					if v and is_instance_valid(v) and v.visible:
						if target_building.has_method("can_house_villager") and target_building.can_house_villager(v):
							target_building.try_house_villager(v)
		...
		"attack":
			var target: Node = _find_entity_by_net_id(int(cmd.get("target_net_id", -1)))
			if target:
				for nid in net_ids:
					var v: Node = _find_villager_by_net_id(int(nid))
					if v and str(v.color_type) == "red":
...
func _apply_house_command(cmd_villagers: Array) -> void:
	...
	for building in homes + churches + town_halls:
		var to_release: Array = []
		for v in building.sheltered:
			if is_instance_valid(v) and v in cmd_villagers:
				to_release.append(v)
		for v in to_release:
			if building.has_method("release_villager"):
	...
	for v in cmd_villagers:
		...
		for b in homes + churches + town_halls:
			...
			if d < best_bd:
				...
				best_building = b
		if best_building:
			best_building.try_house_villager(v)
			TutorialManager.on_shelter()
...
func _room_id_at(pos: Vector2) -> int:
	## O(1) spatial grid lookup with fallback.
	var cx: int = int(pos.x / _room_grid_cell_size)
	var cy: int = int(pos.y / _room_grid_cell_size)
	var key: int = cx * 100000 + cy
	if _room_grid.has(key):
		return _room_grid[key]
	# Fallback: slow scan (shouldn't happen often after grid is built)
	for room in rooms:
...
func _build_room_grid() -> void:
	## Precompute spatial grid: each pixel-space cell maps to a room_id.
	_room_grid.clear()
	var cs: float = _room_grid_cell_size
	for room in rooms:
		var rect: Rect2 = room.get_rect()
		var rid: int = int(room.room_id)
		var cx_start: int = int(rect.position.x / cs)
		var cx_end: int = int(rect.end.x / cs) + 1
		var cy_start: int = int(rect.position.y / cs)
		var cy_end: int = int(rect.end.y / cs) + 1
		for cx in range(cx_start, cx_end):
			for cy in range(cy_start, cy_end):
				_room_grid[cx * 100000 + cy] = rid


func _get_clicked_building(click_pos: Vector2, buildings: Array) -> Node:
...
func _update_brain_context() -> void:
	...
	# Build per-room building lists and adjacent room centers (for idle behavior)
	var room_building_cache: Dictionary = {}  # rid -> Array[Node]
	var all_bldgs: Array = []
	all_bldgs.append_array(homes)
	all_bldgs.append_array(churches)
	all_bldgs.append_array(banks)
	...
	for b in all_bldgs:
		...
		if not room_building_cache.has(brid):
			room_building_cache[brid] = []
		room_building_cache[brid].append(b)

	var room_adj_centers: Dictionary = {}  # rid -> Array[Vector2]
	for door in doorway_info:
		if not door["is_open"]:
			continue
		var ra: int = door["room_a"]
		var rb: int = door["room_b"]
		if room_map.has(rb):
			if not room_adj_centers.has(ra):
				room_adj_centers[ra] = []
			room_adj_centers[ra].append(room_map[rb].get_rect().get_center())
		if room_map.has(ra):
			if not room_adj_centers.has(rb):
				room_adj_centers[rb] = []
	...
	for v in villagers:
		...
		v.brain_buildings = room_building_cache.get(rid, [])
		v.brain_room_centers = room_adj_centers.get(rid, [])
	...
	for v in villagers:
		if v.has_waypoint:
			if v.global_position.distance_to(v.waypoint_target_pos) < float(v.radius) + 20.0:
				v.has_waypoint = false
				# Auto-assign to this room for looping work
				if v.assigned_room_id < 0:
					v.assigned_room_id = v.current_room_id
...
func _process_stone_pickups() -> void:
	...
	for c in collectables:
		...
		for v in room_villagers.get(crid, []):
			if c.try_collect(v):
				break
	for c in rm:
...
func _process_fish_pickups() -> void:
	...
	for f in fish_spots:
		...
		for v in room_villagers.get(frid, []):
			if f.try_collect(v):
				break
	for f in rm:
		fish_spots.erase(f)
...
func _on_phase_changed(is_daytime: bool) -> void:
	...
	if is_daytime:
		...
		# University training: buff sheltered reds at dawn
		for u in universities:
			u.apply_training()
			var trained_count: int = 0
			for v in u.sheltered:
				if is_instance_valid(v) and str(v.color_type) == "red":
					trained_count += 1
			if trained_count > 0:
				EventFeed.push("%d red villager(s) trained at university." % trained_count, Color(0.9, 0.6, 0.2))
		for v in villagers:
...
func _auto_shelter_villagers() -> void:
	for v in villagers:
		...
		var best_building: Node = null
		var best_d: float = INF
		# Check all housing buildings
		var all_housing: Array = []
		all_housing.append_array(homes)
		all_housing.append_array(churches)
		all_housing.append_array(universities)
		all_housing.append_array(farms)
		all_housing.append_array(town_halls)
		for bld in all_housing:
			if bld.has_method("can_house_villager") and not bld.can_house_villager(v):
				continue
			var d: float = v.global_position.distance_to(bld.global_position)
			if d < best_d:
				best_d = d
				best_building = bld
		if best_building:
...
func _on_villager_shifted(villager, old_color, new_color, spawn_count, faction_override) -> void:
	...
	var vname: String = villager.get_display_name() if villager.has_method("get_display_name") else "A villager"
	EventFeed.push("%s joined %s." % [vname, cname], ColorRegistry.get_def(str(new_color)).get("display_color", Color.WHITE))
...
func _spawn_villager(color_id: String, pos: Vector2, p_faction_id: int = 0, parent: Node = null) -> void:
	...
	v.net_id = _next_net_id
	_next_net_id += 1
	if parent != null and parent.villager_name != "":
		var lineage: Dictionary = _gen_descendant_name(parent)
		v.villager_name = lineage["name"]
		v.generation = lineage["generation"]
	else:
		_assign_default_name(v)
	v.resource_dropped.connect(_on_villager_dropped_resource)
	villagers.append(v)
...
func _update_hud() -> void:
	...
	if _player.has_building_selection():
		...
		if b in churches:
			...
			b_type = "Bank"
		elif b in fishing_huts:
			b_type = "Fishing Hut"
		elif b in universities:
			b_type = "University"
		elif b in farms:
			b_type = "Farm"
		elif b in town_halls:
			b_type = "Town Hall"
...
func _process_room_respawns(delta: float) -> void:
	## Respawn stone in stone/quarry rooms and fish in river rooms every 30s.
	## Diamond rooms do NOT respawn.
	_room_respawn_timer += delta
	if _room_respawn_timer < ROOM_RESPAWN_INTERVAL:
		return
	_room_respawn_timer -= ROOM_RESPAWN_INTERVAL

	if _map_gen == null:
		return

	for rid in _map_gen._room_defs_map:
		var rd: Dictionary = _map_gen._room_defs_map[rid]
		var rtype: String = rd.get("type", "")
		var rpos: Vector2 = _map_gen.room_pixel_pos(rd["col"], rd["row"])
		var rsize: Vector2 = _map_gen.room_pixel_size(rd["cw"], rd["ch"])

		if rtype in ["stone_room", "quarry"]:
			# Count existing stone in this room
			var count: int = 0
			for c in collectables:
				if is_instance_valid(c) and not c.collected and _room_id_at(c.global_position) == rid:
					count += 1
			if count < ROOM_RESPAWN_MAX_STONE:
				var c = _collectable_scene.instantiate()
				_collectables_container.add_child(c)
		...
		elif rtype == "river_room":
			var count: int = 0
			for f in fish_spots:
				if is_instance_valid(f) and not f.collected and _room_id_at(f.global_position) == rid:
					count += 1
			if count < ROOM_RESPAWN_MAX_FISH:
				var f = _fish_scene.instantiate()
				_fish_container.add_child(f)
				f.global_position = Vector2(
					GameRNG.randf_range(rpos.x + 60.0, rpos.x + rsize.x - 60.0),
...
func _process_farm_grain(delta: float) -> void:
	## Farms produce grain collectables over time during daytime.
	if not GameClock.is_daytime:
		return
	for f in farms:
		if not is_instance_valid(f) or f.placed_by_faction < 0:
			continue
		if not f.tick_grain(delta):
			continue
		# Count nearby grain to avoid flooding
		var nearby_grain: int = 0
		for c in collectables:
			if is_instance_valid(c) and not c.collected and c.resource_type == "grain":
				if c.global_position.distance_to(f.global_position) < f.GRAIN_SPAWN_RADIUS:
					nearby_grain += 1
		if nearby_grain >= f.MAX_GRAIN_NEARBY:
			continue
		var grain = _collectable_scene.instantiate()
		grain.resource_type = "grain"
		_collectables_container.add_child(grain)
		var offset := Vector2(
			GameRNG.randf_range(-f.GRAIN_SPAWN_RADIUS, f.GRAIN_SPAWN_RADIUS),
			GameRNG.randf_range(-f.GRAIN_SPAWN_RADIUS, f.GRAIN_SPAWN_RADIUS))
		grain.global_position = f.global_position + offset
		collectables.append(grain)
		f.grain_produced += 1


const PORTAL_TELEPORT_RADIUS := 80.0
const PORTAL_COOLDOWN := 1.5  ## seconds before a villager can re-enter a portal
var _portal_cooldowns: Dictionary = {}  ## villager net_id -> remaining cooldown

func _process_portals() -> void:
	## Teleport villagers (and enemies) near a portal to the partner portal.
	if _portals.is_empty() or _portal_pairs.is_empty():
		return

	# Tick down cooldowns
	var expired: Array = []
	for nid in _portal_cooldowns:
		_portal_cooldowns[nid] -= get_process_delta_time()
		if _portal_cooldowns[nid] <= 0.0:
			expired.append(nid)
	for nid in expired:
		_portal_cooldowns.erase(nid)

	# Build portal position lookup: room_id -> global_position of portal node
	var portal_positions: Dictionary = {}
	for p in _portals:
		if is_instance_valid(p):
			portal_positions[p.room_id] = p.global_position

	# Check villagers
	for v in villagers:
		if not is_instance_valid(v) or not v.visible:
			continue
		if _portal_cooldowns.has(v.net_id):
			continue
		var rid: int = v.current_room_id
		if not _portal_pairs.has(rid):
			continue
		var portal_pos: Vector2 = portal_positions.get(rid, Vector2.ZERO)
		if portal_pos == Vector2.ZERO:
			continue
		if v.global_position.distance_to(portal_pos) > PORTAL_TELEPORT_RADIUS:
			continue
		# Teleport to partner portal
		var partner_rid: int = _portal_pairs[rid]
		var dest_pos: Vector2 = portal_positions.get(partner_rid, Vector2.ZERO)
		if dest_pos == Vector2.ZERO:
			continue
		# Offset slightly so they don't immediately re-trigger
		var offset := Vector2(PORTAL_TELEPORT_RADIUS + 20.0, 0).rotated(
			GameRNG.randf_range(0.0, TAU))
		v.global_position = dest_pos + offset
		_portal_cooldowns[v.net_id] = PORTAL_COOLDOWN
		# Clear any move command so they don't path back
		if v.command_mode == "move_to":
			v.command_mode = "none"
	...
	for e in enemies:
		if not is_instance_valid(e) or e.is_dead:
			continue
		var eid: int = e.net_id
		if _portal_cooldowns.has(eid + 100000):
			continue
		var rid: int = e.current_room_id
		if not _portal_pairs.has(rid):
			continue
		var portal_pos: Vector2 = portal_positions.get(rid, Vector2.ZERO)
		if portal_pos == Vector2.ZERO:
			continue
		if e.global_position.distance_to(portal_pos) > PORTAL_TELEPORT_RADIUS:
			continue
		var partner_rid: int = _portal_pairs[rid]
		var dest_pos: Vector2 = portal_positions.get(partner_rid, Vector2.ZERO)
		if dest_pos == Vector2.ZERO:
		...
		var offset := Vector2(PORTAL_TELEPORT_RADIUS + 20.0, 0).rotated(
			GameRNG.randf_range(0.0, TAU))
		e.global_position = dest_pos + offset
		_portal_cooldowns[eid + 100000] = PORTAL_COOLDOWN
...
const CARRY_TO_IDX := {"": 0, "stone": 1, "fish": 2, "diamond": 3, "grain": 4}
const IDX_TO_CARRY := ["", "stone", "fish", "diamond", "grain"]
...
func _broadcast_snapshot() -> void:
	...
	for fid in FactionManager.get_all_faction_ids():
		eco_data[fid] = [Economy.get_stone(fid), Economy.get_fish(fid), Economy.get_diamonds(fid)]
	snap["eco"] = eco_data
...
func _apply_snapshot(snap: Dictionary) -> void:
	...
	for vd in snap.get("v", []):
		...
		if vd.size() > 13:
			v.villager_name = str(vd[13])
			v.generation = int(vd[14])
	...
	for fid_key in eco:
		...
		if vals.size() >= 3:
			Economy.set_diamonds(fid, int(vals[2]))
	...
	for i in range(mini(ws.size(), walls.size())):
...
func _draw_faction_markers() -> void:
	var faction_positions: Dictionary = {}
	for v in villagers:
		if not is_instance_valid(v) or not v.visible:
			continue
		if v.faction_id < 0:
