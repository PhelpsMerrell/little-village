...
func _get_button_at(pos: Vector2) -> String:
	...
	if Rect2(cx - bw * 0.5, cy - 10, bw, bh).has_point(pos):
		return "new"
	# Continue button
	if SaveManager.has_save() and Rect2(cx - bw * 0.5, cy + 60, bw, bh).has_point(pos):
		return "continue"
	return ""
...
func _draw() -> void:
	...
	if SaveManager.has_save():
		var cont_rect := Rect2(cx - bw * 0.5, cy + 60, bw, bh)
		...
		draw_string(ThemeDB.fallback_font, Vector2(cx - 50, cy + 92),
